...
/// <summary>
/// Full happy-path connection: authenticate, bring tunnel up, wait for
/// stdin close (user disconnect), then tear down cleanly.
/// </summary>
public sealed class SuccessfulConnectScenario : IScenario
{
    public int ExitCode => 0;

    public async Task RunAsync(int delayMs, bool passwordEcho)
    {
        Program.Log("INFO", "Starting openfortivpn-harness (scenario: successful_connect)");
