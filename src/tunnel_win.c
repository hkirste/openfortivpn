			...
			if (cert) {
				...
				if (!found) {
					...
					log_error("Certificate digest: %s\n",
					          digest_str);
					event_emit("cert_error", "\"digest\":\"%s\",\"reason\":\"verification_failed\"", digest_str);
...
static int get_gateway_host_ip(struct tunnel *tunnel)
{
	...
	if (ret != 0 || !result) {
		log_error("Could not resolve host: %s\n",
		          tunnel->config->gateway_host);
		event_emit("error", "\"code\":%d,\"category\":\"dns\",\"message\":\"Could not resolve host\"", OFV_EXIT_DNS_FAILED);
		return 1;
	}
...
static int on_ppp_if_up(struct tunnel *tunnel)
{
...

	{
		...
		log_info("Assigned IP: %s\n", ip_str);
		{
			...
			event_emit("tunnel_up", "\"local_ip\":\"%s\",\"dns1\":\"%s\",\"dns2\":\"%s\"", ip_str, dns1_str, dns2_str);
		}
	}
